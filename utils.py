...
def sticker_type_properties(sticker_type: StickerType):
    ...
    is_emoji = False
    if sticker_type in [StickerType.ANIMATED_STICKER, StickerType.POPUP_STICKER, StickerType.ANIMATED_AND_SOUND_STICKER,
                        StickerType.POPUP_AND_SOUND_STICKER, StickerType.ANIMATED_EMOJI]:
        has_animation = True
...
STICKER_SET_URL_REGEX = {
    SourceUrlType.LINE: (re.compile(r'https://store.line.me/stickershop/product/(\d+)/?(\w+)?'), False),
    SourceUrlType.LINE_EMOJI: (re.compile(r'https://store.line.me/emojishop/product/([a-f0-9]+)/?(\w+)?'), True),
    SourceUrlType.YABE: (re.compile(r'https://yabeline.tw/Stickers_Data.php\?Number=(\d+)'), False),
    SourceUrlType.YABE_EMOJI: (re.compile(r'https://yabeline.tw/Emoji_Data.php\?Number=(\d+)'), True),
}
...
EMOJI_SET_META_URL = 'https://stickershop.line-scdn.net/sticonshop/v1/{pack_id}/sticon/iphone/meta.json'
# sticker zip archive
