...
def get_real_pack_id_from_yabe_emoji(pack_id):
    r = requests.get(STICKER_SET_URL_TEMPLATES[SourceUrlType.YABE_EMOJI].format(pack_id=pack_id), proxies=_proxies,
...
def get_sticker_info_from_line_page(pack_id, is_emoji, lang):
    url = STICKER_SET_URL_TEMPLATES[SourceUrlType.LINE_EMOJI].format(pack_id=pack_id, lang=lang) if is_emoji else \
        STICKER_SET_URL_TEMPLATES[SourceUrlType.LINE].format(pack_id=pack_id, lang=lang)
    r = requests.get(url, proxies=_proxies, headers=FAKE_HEADERS)
    soup = BeautifulSoup(r.content, 'html5lib')
    if soup.select_one('[data-test="not-on-sale-description"]'):
        ...
        print(f'WARNING: Pack {pack_id} is not on sale, title: {title}, skipping author data')
...
def get_metadata(pack_id: str, is_emoji: bool) -> dict:
    ...
    if r.status_code == 404:
        raise FileNotFoundError(f'Sticker pack {pack_id} not found!')
    return r.json()
